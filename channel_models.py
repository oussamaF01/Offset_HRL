...
def _load_numeric_csv(filename, delimiter=",", skip_header=0):
...
class SINRSelectiveFading:
    ...
    def __init__(self, rng, model_name, n_prbs = 100, filenames = FILESNAMES, user_ids = None):
        ...
        for filename in filenames:
            sample_matrix = _load_numeric_csv(filename)
            ...
            if n_prbs > 100:
                ...
                self.samples.append(sample_matrix[:n_prbs, :])
...
class SNRGenerator:
    ...
    '''
    def __init__(self, rng, filename = DATASET_DIR / "srslte_v19.03.csv", user_ids = None, powers = None):
        # random number generator
        self.rng = rng

        # extract the samples from file
        data = np.genfromtxt(filename, delimiter=",", names=True)
        self.norm_snr_array = np.asarray(data["mean_snr"], dtype=float).flatten() - np.asarray(data["txpower"], dtype=float).flatten()
...
class MCSCodeset:
    ...
    '''
    def __init__(self, filename=DATASET_DIR / "mcs_codeset.csv"):
        data = np.genfromtxt(filename, delimiter=",", names=True, dtype=None, encoding="utf-8")
        self.rate = np.asarray(data["rate"], dtype=float).flatten()
        self.snr = np.asarray(data["snr"], dtype=float).flatten()
        self.order = np.asarray(data["order"], dtype=float).flatten()
        self.modulation = np.asarray(data["modulation"]).flatten()
